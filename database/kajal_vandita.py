...
title=Label(root,text='STUDENT MANAGEMENT SYSTEM',bd=10,relief=GROOVE,font=('ALGERIAN',40,'bold'),bg='teal',fg='white')
...
m_title=Label(manage_frame,text='MANAGE STUDENT',bg='lightseagreen',fg='white',font=('times new roman',26,'bold'))
m_title.grid(row=0,columnspan=2,pady=20)
        
lbl_roll=Label(manage_frame,text='ROLL NO',bg='lightseagreen',fg='white',font=('times new roman',20,'bold'))
lbl_roll.grid(row=1,column=0,pady=10,padx=20,sticky='w')
        
txt_roll=Entry(manage_frame,textvariable=rollno_var,font=('times new roman',15,'bold'),bd=5,relief=GROOVE)
...
lbl_address=Label(manage_frame,text='ADDRESS',bg='lightseagreen',fg='white',font=('times new roman',20,'bold'))
...
def add_student():
    con=mysql.connector.connect(host='localhost',
                                ...
                                database='stms')
    cur=con.cursor()
    cur.execute('insert into student values(%s,%s,%s,%s,%s,%s,%s)',(rollno_var.get(),
                                                                    name_var.get(),
                                                                    email_var.get(),
                                                                    gender_var.get(),
                                                                    contact_var.get(),
...
def delete_data():
    ...
    cur=con.cursor()
    cur.execute('delete from student where rollno=%s',(rollno_var.get(),))
    con.commit()
    con.close()
    fetch_data()
    clear()
def search_data():
    ...
    cur.execute('select*from student where rollno=%s',(search_txt.get(),))
    rows=cur.fetchall()
    if len(rows)!=0:
        for item in student_table.get_children():
            student_table.delete(item)
...
student_table=ttk.Treeview(table_frame,columns=('roll no','name','email','gender','contact','D.O.B','address'),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
